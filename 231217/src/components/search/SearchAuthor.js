...
import searchListAuthor from "./searchList.module.css"; // 변경된 부분
...
const SearchTitle = ({ setSearchFail, setSearchFail2 }) => {
  ...
  return (
    <div className={searchListAuthor.searchLists}>
      <div className={searchListAuthor.pointtop}></div>

      <form className={searchListAuthor.forms}>
        <button onClick={handleSearch}>
          <img src={`${publicUrl}/images/submit.svg`} alt="초기화" />
        </button>
        <input
          className={searchListAuthor.search}
          type="text"
          value={searchQuery}
      ...
      <ul className={searchListAuthor.searchList}>
        {searchResults.map((result) => (
          <li key={result.isbn}>
            <div
              ...
              className={searchListAuthor.imgBg}
            ></div>
